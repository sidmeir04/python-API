...
class get_functions():
    ...
    @staticmethod
    def get_contact(connection, id=None, staff='', caller_name='', caller_email='', call_date=None, phone_number='', referral_type='', additional_notes='', tour_scheduled=None, tour_not_scheduled_reason='', follow_up_date=None):
        ...
        if additional_notes:
            query += " AND additional_notes LIKE %s"
            filters.append(f'%{additional_notes}%')
        ...
        if tour_not_scheduled_reason:
            query += " AND tour_not_scheduled_reason LIKE %s"
            filters.append(f'%{tour_not_scheduled_reason}%')
    ...
    @staticmethod
    def get_tour(connection, id=None, tour_date=None, attended=None, no_join_reason='', clinicians='', attendees='', interactions='', strategies_used=None, aep_deadline=None, joined_after=None, likely_to_join=None, additional_notes='', canceled=None, cancel_reason=''):
        cursor = connection.cursor()
        tour_date = string_to_date(tour_date)
        aep_deadline = string_to_date(aep_deadline)
        # Start building the base query
        ...
        if no_join_reason:
            query += " AND no_join_reason LIKE %s"
            filters.append(f'%{no_join_reason}%')
        ...
        if attendees:
            query += " AND attendees LIKE %s"
            filters.append(f'%{attendees}%')

        if interactions:
            query += " AND interactions LIKE %s"
            filters.append(f'%{interactions}%')
        ...
        if cancel_reason:
            query += " AND cancel_reason LIKE %s"
            filters.append(f'%{cancel_reason}%')
        ...
        dict_results = {}
        columns = ["id", "tour_date", "attended", "no_join_reason", "clinicians", "attendees", "interactions", "strategies_used", "aep_deadline", "joined_after", "likely_to_join", "additional_notes", "canceled", "cancel_reason"]
        for i in range(len(columns)):
            dict_results[columns[i]] = [str(results[j][i]) for j in range(len(results))]
        if connection:
            connection.close()
    ...
    @staticmethod
    def get_membership_enrollment_form(connection, id=None, sexual_orientation='', race='', income=None, living_status='', grew_up='', occupations='', prev_speech_therapy='', other_therapy='', hearing_loss=None, hearing_aid=None, aphasia_cause='', aphasia_onset=None, brain_location='', medications='', filled_by='', completed_date=None, patient_info=None):
        cursor = connection.cursor()
        aphasia_onset = string_to_date(aphasia_onset)
        completed_date = string_to_date(completed_date)
        # Start building the base query
        ...
        if living_status:
            query += " AND living_status LIKE %s"
            filters.append(f'%{living_status}%')
        ...
        if prev_speech_therapy:
            query += " AND prev_speech_therapy LIKE %s"
            filters.append(f'%{prev_speech_therapy}%')

        if other_therapy:
            query += " AND other_therapy LIKE %s"
            filters.append(f'%{other_therapy}%')
        ...
        if patient_info is not None:
            query += " AND patient_info = %s"
            filters.append(patient_info)
    ...
    @staticmethod
    def get_medical_history_form(connection, id=None, physician_name='', specialty='', physician_address='', physician_phone='', aphasia_cause='', aphasia_onset=None, stroke_location='', lesion_location='', primary_diagnosis='', secondary_diagnosis='', seizure_history=None, last_seizure_date=None, anti_seizure_med=None, visual_impairments='', visual_field_cut=None, other_visual_impairments='', completion_date=None, other_conditions=None):
        cursor = connection.cursor()
        aphasia_onset = string_to_date(aphasia_onset)
        last_seizure_date = string_to_date(last_seizure_date)
        completion_date = string_to_date(completion_date)
        # Start building the base query
        ...
        if stroke_location:
            query += " AND stroke_location LIKE %s"
            filters.append(f'%{stroke_location}%')
        ...
        if other_conditions is not None:
            query += " AND other_conditions = %s"
            filters.append(other_conditions)
        ...
        dict_results = {}
        columns = ["id", "physician_name", "specialty", "physician_address", "physician_phone", "aphasia_cause", "aphasia_onset", "stroke_location", "lesion_location", "primary_diagnosis", "secondary_diagnosis", "seizure_history", "last_seizure_date", "anti_seizure_med", "visual_impairments", "visual_field_cut", "other_visual_impairments", "completion_date", "other_conditions"]
        for i in range(len(columns)):
            if columns[i] in ["other_conditions"]:
                dict_results[columns[i]] = [
                    json.loads(results[j][i]) if results[j][i] and results[j][i] != "None" else None
                    for j in range(len(results))
                ]
    ...
    @staticmethod
    def get_incident_report(connection, id=None, incident_date=None, incident_location='', persons_involved='', description='', action_taken=''):
        cursor = connection.cursor()
        incident_date = string_to_date(incident_date)
        # Start building the base query
        ...
        if persons_involved:
            query += " AND persons_involved LIKE %s"
            filters.append(f'%{persons_involved}%')
        ...
        if action_taken:
            query += " AND action_taken LIKE %s"
            filters.append(f'%{action_taken}%')
    ...
    @staticmethod
    def get_evaluation(connection, id=None, completed=None, administerer='', test_type='', date_administered=None):
        cursor = connection.cursor()
        date_administered = string_to_date(date_administered)
        # Start building the base query
        ...
        if test_type:
            query += " AND test_type LIKE %s"
            filters.append(f'%{test_type}%')
    ...
    @staticmethod
    def get_transportation_information(connection, id=None, am_name='', am_phone='', pm_name='', pm_phone='', transportation_notes=''):
        cursor = connection.cursor()
        # Start building the base query
        ...
        if transportation_notes:
            query += " AND transportation_notes LIKE %s"
            filters.append(f'%{transportation_notes}%')
    ...
    @staticmethod
    def get_emergency_contact(connection, id=None, name='', relationship='', day_phone='', evening_phone='', cell_phone='', email='', address='', completion_date=None):
        cursor = connection.cursor()
        completion_date = string_to_date(completion_date)
        # Start building the base query
    ...
    @staticmethod
    def get_member(connection, id=None, name='', dob=None, email='', aep_completion_date=None, join_date=None, schedule=None, phone='', address='', county='', gender='', veteran=None, joined=None, caregiver_needed=None, adler_program='', member_type='', date_changed=None, notes='', member_info=None, enrollment_form=None, medical_history=None, emergency_contact_one=None, emergency_contact_two=None, transport_info=None):
        cursor = connection.cursor()
        dob = string_to_date(dob)
        aep_completion_date = string_to_date(aep_completion_date)
        join_date = string_to_date(join_date)
        date_changed = string_to_date(date_changed)
        # Start building the base query
        ...
        if adler_program:
            query += " AND adler_program LIKE %s"
            filters.append(f'%{adler_program}%')

        if member_type:
            query += " AND member_type LIKE %s"
            filters.append(f'%{member_type}%')
        ...
        if notes:
            ...
            filters.append(f'%{notes}%')

        if member_info is not None:
            query += " AND member_info = %s"
            filters.append(member_info)
    ...
    @staticmethod
    def get_applications(connection, id=None, birthday=None, occupation='', is_slp=None, relevant_experience='', education='', interests_skills_hobbies='', languages_spoken='', will_substitute=None, convicted_of_crime=None, application_date=None):
        cursor = connection.cursor()
        birthday = string_to_date(birthday)
        application_date = string_to_date(application_date)
        # Start building the base query
        ...
        if relevant_experience:
            query += " AND relevant_experience LIKE %s"
            filters.append(f'%{relevant_experience}%')

        if education:
            query += " AND education LIKE %s"
            filters.append(f'%{education}%')

        if interests_skills_hobbies:
            query += " AND interests_skills_hobbies LIKE %s"
            filters.append(f'%{interests_skills_hobbies}%')
    ...
    @staticmethod
    def get_volunteer(connection, id=None, name='', phone='', address='', email='', referral_source='', background_check_date=None, video_watched_date=None, emergency_contacts='', media_release=None, confidentiality=None, training_level=None, resumue=None):
        cursor = connection.cursor()
        background_check_date = string_to_date(background_check_date)
        video_watched_date = string_to_date(video_watched_date)
        # Start building the base query
        ...
        if referral_source:
            query += " AND referral_source LIKE %s"
            filters.append(f'%{referral_source}%')
        ...
        if emergency_contacts:
            query += " AND emergency_contacts LIKE %s"
            filters.append(f'%{emergency_contacts}%')
        ...
        if resumue is not None:
            query += " AND resumue = %s"
            filters.append(resumue)
        ...
        dict_results = {}
        columns = ["id", "name", "phone", "address", "email", "referral_source", "background_check_date", "video_watched_date", "emergency_contacts", "media_release", "confidentiality", "training_level", "resumue"]
        for i in range(len(columns)):
            dict_results[columns[i]] = [str(results[j][i]) for j in range(len(results))]
        if connection:
            connection.close()
    ...
    @staticmethod
    def get_caregiver(connection, id=None, name='', phone='', email='', relationship='', date_contacted=None, notes='', group_attending='', attending=None, caregiver_type='', sex='', race='', occupations='', support_group=None, covid_vaccine_date=None, allergies='', medications='', participation='', robly=None, enrollment_form=None, medical_history=None, emergency_contact_one=None, emergency_contact_two=None, transport_info=None, member_id=None):
        cursor = connection.cursor()
        date_contacted = string_to_date(date_contacted)
        covid_vaccine_date = string_to_date(covid_vaccine_date)
        # Start building the base query
        ...
        if sex:
            query += " AND sex LIKE %s"
            filters.append(f'%{sex}%')
    ...
    @staticmethod
    def get_outreach(connection, id=None, contacted_date=None, staff_contacted='', organization='', org_type='', outreach_type='', target_location='', num_people=None, robly=None, notes=''):
        cursor = connection.cursor()
        contacted_date = string_to_date(contacted_date)
        # Start building the base query
...
class update_functions():
    ...
    @staticmethod
    def update_contact(connection, id, staff=None, caller_name=None, caller_email=None, call_date=None, phone_number=None, referral_type=None, additional_notes=None, tour_scheduled=None, tour_not_scheduled_reason=None, follow_up_date=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_tour(connection, id, tour_date=None, attended=None, no_join_reason=None, clinicians=None, attendees=None, interactions=None, strategies_used=None, aep_deadline=None, joined_after=None, likely_to_join=None, additional_notes=None, canceled=None, cancel_reason=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_membership_enrollment_form(connection, id, sexual_orientation=None, race=None, income=None, living_status=None, grew_up=None, occupations=None, prev_speech_therapy=None, other_therapy=None, hearing_loss=None, hearing_aid=None, aphasia_cause=None, aphasia_onset=None, brain_location=None, medications=None, filled_by=None, completed_date=None, patient_info=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_medical_history_form(connection, id, physician_name=None, specialty=None, physician_address=None, physician_phone=None, aphasia_cause=None, aphasia_onset=None, stroke_location=None, lesion_location=None, primary_diagnosis=None, secondary_diagnosis=None, seizure_history=None, last_seizure_date=None, anti_seizure_med=None, visual_impairments=None, visual_field_cut=None, other_visual_impairments=None, completion_date=None, other_conditions=None):
        cursor = connection.cursor()
        ...
        if other_conditions is not None:
            ...
            update_values.append(other_conditions)
    ...
    @staticmethod
    def update_incident_report(connection, id, incident_date=None, incident_location=None, persons_involved=None, description=None, action_taken=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_evaluation(connection, id, completed=None, administerer=None, test_type=None, date_administered=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_transportation_information(connection, id, am_name=None, am_phone=None, pm_name=None, pm_phone=None, transportation_notes=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_emergency_contact(connection, id, name=None, relationship=None, day_phone=None, evening_phone=None, cell_phone=None, email=None, address=None, completion_date=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_member(connection, id, name=None, dob=None, email=None, aep_completion_date=None, join_date=None, schedule=None, phone=None, address=None, county=None, gender=None, veteran=None, joined=None, caregiver_needed=None, adler_program=None, member_type=None, date_changed=None, notes=None, member_info=None, enrollment_form=None, medical_history=None, emergency_contact_one=None, emergency_contact_two=None, transport_info=None):
        cursor = connection.cursor()
        ...
        if adler_program:
            update_query += "adler_program = %s, "
            update_values.append(adler_program)
        ...
        if member_info is not None:
            update_query += "member_info = %s, "
            update_values.append(member_info)
    ...
    @staticmethod
    def update_applications(connection, id, birthday=None, occupation=None, is_slp=None, relevant_experience=None, education=None, interests_skills_hobbies=None, languages_spoken=None, will_substitute=None, convicted_of_crime=None, application_date=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_volunteer(connection, id, name=None, phone=None, address=None, email=None, referral_source=None, background_check_date=None, video_watched_date=None, emergency_contacts=None, media_release=None, confidentiality=None, training_level=None, resumue=None):
        cursor = connection.cursor()
        ...
        if resumue is not None:
            update_query += "resumue = %s, "
            update_values.append(resumue)
    ...
    @staticmethod
    def update_caregiver(connection, id, name=None, phone=None, email=None, relationship=None, date_contacted=None, notes=None, group_attending=None, attending=None, caregiver_type=None, sex=None, race=None, occupations=None, support_group=None, covid_vaccine_date=None, allergies=None, medications=None, participation=None, robly=None, enrollment_form=None, medical_history=None, emergency_contact_one=None, emergency_contact_two=None, transport_info=None, member_id=None):
        cursor = connection.cursor()
    ...
    @staticmethod
    def update_outreach(connection, id, contacted_date=None, staff_contacted=None, organization=None, org_type=None, outreach_type=None, target_location=None, num_people=None, robly=None, notes=None):
        cursor = connection.cursor()
...
class insert_functions():
    ...
    @staticmethod
    def insert_contact(connection, staff, caller_name, caller_email, call_date, phone_number, referral_type, additional_notes, tour_scheduled, tour_not_scheduled_reason, follow_up_date):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Contact (
            staff, caller_name, caller_email, call_date, phone_number, referral_type,
            additional_notes, tour_scheduled, tour_not_scheduled_reason, follow_up_date
        ) 
    ...
    @staticmethod
    def insert_tour(connection, tour_date, attended, no_join_reason, clinicians, attendees, interactions, strategies_used, aep_deadline, joined_after, likely_to_join, additional_notes, canceled, cancel_reason):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Tour (
            tour_date, attended, no_join_reason, clinicians, attendees, interactions,
            strategies_used, aep_deadline, joined_after, likely_to_join, additional_notes, canceled,
            cancel_reason
        ) 
    ...
    @staticmethod
    def insert_membership_enrollment_form(connection, sexual_orientation, race, income, living_status, grew_up, occupations, prev_speech_therapy, other_therapy, hearing_loss, hearing_aid, aphasia_cause, aphasia_onset, brain_location, medications, filled_by, completed_date, patient_info):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Membership_Enrollment_Form (
            sexual_orientation, race, income, living_status, grew_up, occupations,
            prev_speech_therapy, other_therapy, hearing_loss, hearing_aid, aphasia_cause, aphasia_onset,
            brain_location, medications, filled_by, completed_date, patient_info
        ) 
    ...
    @staticmethod
    def insert_medical_history_form(connection, physician_name, specialty, physician_address, physician_phone, aphasia_cause, aphasia_onset, stroke_location, lesion_location, primary_diagnosis, secondary_diagnosis, seizure_history, last_seizure_date, anti_seizure_med, visual_impairments, visual_field_cut, other_visual_impairments, completion_date, other_conditions):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Medical_History_Form (
            physician_name, specialty, physician_address, physician_phone, aphasia_cause, aphasia_onset,
            stroke_location, lesion_location, primary_diagnosis, secondary_diagnosis, seizure_history, last_seizure_date,
            anti_seizure_med, visual_impairments, visual_field_cut, other_visual_impairments, completion_date, other_conditions
        ) 
    ...
    def insert_emergency_contact(connection, name, relationship, day_phone, evening_phone, cell_phone, email, address, completion_date):
        ...
        INSERT INTO Emergency_Contact (
            name, relationship, day_phone, evening_phone, cell_phone, email,
    ...
    @staticmethod
    def insert_member(connection, name, dob, email, aep_completion_date, join_date, schedule, phone, address, county, gender, veteran, joined, caregiver_needed, adler_program, member_type, date_changed, notes, member_info, enrollment_form, medical_history, emergency_contact_one, emergency_contact_two, transport_info):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Member (
            name, dob, email, aep_completion_date, join_date, schedule,
            phone, address, county, gender, veteran, joined,
            caregiver_needed, adler_program, member_type, date_changed, notes, member_info,
            enrollment_form, medical_history, emergency_contact_one, emergency_contact_two, transport_info
        ) 
    ...
    @staticmethod
    def insert_applications(connection, birthday, occupation, is_slp, relevant_experience, education, interests_skills_hobbies, languages_spoken, will_substitute, convicted_of_crime, application_date):
        cursor = connection.cursor()
        insert_query = """
    ...
    @staticmethod
    def insert_volunteer(connection, name, phone, address, email, referral_source, background_check_date, video_watched_date, emergency_contacts, media_release, confidentiality, training_level, resumue):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Volunteer (
            name, phone, address, email, referral_source, background_check_date,
            video_watched_date, emergency_contacts, media_release, confidentiality, training_level, resumue
        ) 
    ...
    @staticmethod
    def insert_caregiver(connection, name, phone, email, relationship, date_contacted, notes, group_attending, attending, caregiver_type, sex, race, occupations, support_group, covid_vaccine_date, allergies, medications, participation, robly, enrollment_form, medical_history, emergency_contact_one, emergency_contact_two, transport_info, member_id):
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO Caregiver (
            name, phone, email, relationship, date_contacted, notes,
            group_attending, attending, caregiver_type, sex, race, occupations,
            support_group, covid_vaccine_date, allergies, medications, participation, robly,
            enrollment_form, medical_history, emergency_contact_one, emergency_contact_two, transport_info, member_id
        ) 
    ...
    @staticmethod
    def insert_outreach(connection, contacted_date, staff_contacted, organization, org_type, outreach_type, target_location, num_people, robly, notes):
        cursor = connection.cursor()
        insert_query = """
